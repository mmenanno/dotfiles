...
- **`bin/shared` helpers**: Beyond logging, it provides `run_with_timeout <secs> <cmd>` (timeout/gtimeout fallback), `confirm <prompt> [timeout]` (y/N, declines on timeout or no tty), and `start_sudo_keepalive`/`stop_sudo_keepalive`. Set `USE_ICON_STYLE=true` explicitly — inheriting it from a sourced `bin/dv` only works in zsh.
- **Never wrap an external call that can block**: any CLI that talks to a daemon (notably `docker info`) can hang forever with no client-side timeout. Use `run_with_timeout`, and bound polling loops with a `SECONDS` deadline rather than summing `sleep` durations. See `wait_for_docker` in `bin/clean`.
- **Traps must re-raise, not return**: a bash `INT` handler that returns lets execution resume, making the script unkillable by Ctrl-C. Disarm (`trap - INT TERM EXIT`), tear down, then `kill -INT $$`. Also save real stderr up front (`exec {FD}>&2`) — a trap firing during a `&>/dev/null` call inherits that redirection and silently swallows teardown output. See `on_signal`/`do_cleanup` in `bin/clean`.
- **`df /` can't measure cleanup**: on APFS `/` is the sealed read-only system snapshot, so its used-space never moves. Measure *available* space on `$HOME` (`/System/Volumes/Data`) instead.
- **`grep -c` with no matches**: prints `0` *and* exits 1, so `$(grep -c … || echo 0)` yields `"0\n0"` and breaks later `[[ … -gt 0 ]]`/`$(( … ))`. Assign then default: `n=$(grep -c …) || n=0`.
